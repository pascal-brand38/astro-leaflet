...
type _googleExtraType =
  | ''
  | `&extra=${_googleExtraValueType}`
  | `&extra=${_googleExtraValueType},${_googleExtraValueType}`;
...
export function getLayerOptionsFromName(name: LayerNamesType): LayerFromNameType {
  ...
  if (name.startsWith('Michelin')) {
    ...
    let tileLayer =
      'https://map1.viamichelin.com/map/mapdirect?map=viamichelin&z={z}&x={x}&y={y}&format=png&version=202505301130&layer=background&protocol=https';
    if (searchParams.has('type') && searchParams.get('type') === 'label') {
      tileLayer =
        'https://{s}.viamichelin.com/map/mapdirect?map=hybrid&z={z}&x={x}&y={y}&format=png&version=202505301130&layer=network';
    }
    return {
      tileLayer,
      options: {
